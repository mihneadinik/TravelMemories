...
class HomeViewModel : ViewModel() {
    ...
    private val _memoriesList: MutableLiveData<List<Memory>> = MutableLiveData(listOf())
    val memoriesList: List<Memory>?
        get() = _memoriesList.value

    fun addNewMemory(memory: Memory) {
        (_memoriesList.value as MutableList<Memory>).add(memory)
    }
